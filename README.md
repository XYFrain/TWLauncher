...
启动器固定从程序同级目录�?`Game` 文件夹读�?Minecraft 1.12.2，不会使�?`%AppData%\\.minecraft`。请准备以下文件�?
```text
Game/
  Minecraft.json
  Minecraft.jar
...
点击“启动游戏”会以离线用户名 `Player` 启动。可选地，将 Java 8 放在 `Game/java/bin/javaw.exe`；否则启动器会使用系�?PATH 中的 `javaw.exe`�?
一个基�?WPF�?NET Framework 4.8）打造的启动器界面�?
...
├── Properties/          # 程序集属�?& 资源文件
├── App.xaml / .cs       # 应用程序入口
├── MainWindow.xaml / .cs # 主窗�?
